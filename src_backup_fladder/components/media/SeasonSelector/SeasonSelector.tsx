...
import { Dropdown, LazyImage, Skeleton } from '../../ui';
  ...
  return (
    <div className={styles['season-selector']}>
      <div className={styles['season-header']}>
        ...
        <Dropdown
          trigger={<button className="px-4 py-2 bg-gray-800 rounded-md text-white">{seasons.find(s => s.Id === activeSeasonId)?.Name || '选择季'}</button>}
          items={seasonOptions.map(opt => ({
            id: opt.value,
            label: opt.label,
        ...
        {loadingEpisodes
          ? Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className={styles['episode-card']}>
                <Skeleton variant="custom" width={160} height="100%" borderRadius={0} />
                <div style={{ padding: 12, flex: 1 }}>
                  <Skeleton variant="text" width="40%" />
                  <Skeleton variant="title" width="80%" style={{ marginTop: 8 }} />
                  <Skeleton variant="text" width="100%" style={{ marginTop: 8 }} />
                </div>
              </div>
            ))
