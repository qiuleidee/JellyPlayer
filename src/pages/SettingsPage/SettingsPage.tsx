...
function SubtitleSettings() {
  const { subtitleSize, subtitleColor, subtitleBackground, updateSettings } = useSettingsStore();

  const sizeLabel = { small: '小', normal: '标准', large: '大', xlarge: '超大' };
  const colorLabel: Record<string, string> = { 
    '#FFFFFF': '白色', '#FFFF00': '黄色', '#00FFFF': '青色', '#00FF00': '绿色'
  };
  const bgLabel: Record<string, string> = {
    'transparent': '无', 'rgba(0, 0, 0, 0.5)': '半透明黑色', 'rgba(0, 0, 0, 0.8)': '深黑色', 'rgba(0, 0, 0, 1)': '纯黑色'
  };
  ...
  return (
    <div>
      ...
      <div className={styles['form-group']}>
        <label className={styles['form-label']}>字幕颜色</label>
        <Dropdown
          trigger={<Button variant="secondary">{colorLabel[subtitleColor] || '白色'}</Button>}
          items={[
            { id: 'c1', label: '白色', onClick: () => updateSettings({ subtitleColor: '#FFFFFF' }) },
            { id: 'c2', label: '黄色', onClick: () => updateSettings({ subtitleColor: '#FFFF00' }) },
            { id: 'c3', label: '青色', onClick: () => updateSettings({ subtitleColor: '#00FFFF' }) },
            { id: 'c4', label: '绿色', onClick: () => updateSettings({ subtitleColor: '#00FF00' }) },
          ]}
          align="left"
        />
      </div>
      ...
      <div className={styles['form-group']}>
        <label className={styles['form-label']}>字幕底色</label>
        <Dropdown
          trigger={<Button variant="secondary">{bgLabel[subtitleBackground] || '半透明黑色'}</Button>}
          items={[
            { id: 'b1', label: '无', onClick: () => updateSettings({ subtitleBackground: 'transparent' }) },
            { id: 'b2', label: '半透明黑色', onClick: () => updateSettings({ subtitleBackground: 'rgba(0, 0, 0, 0.5)' }) },
            { id: 'b3', label: '深黑色', onClick: () => updateSettings({ subtitleBackground: 'rgba(0, 0, 0, 0.8)' }) },
            { id: 'b4', label: '纯黑色', onClick: () => updateSettings({ subtitleBackground: 'rgba(0, 0, 0, 1)' }) },
          ]}
          align="left"
        />
      </div>
...
function NetworkSettings() {
  ...
  const setActiveServer = useAuthStore((s) => s.setActiveServer);
              ...
              {activeServerId !== server.id && (
                <Button 
                  ...
                  onClick={() => {
                    ...
                    window.location.href = '/'; // 强制刷新重载新服务器数据
                  }}
                >
                  切换至此服务器
                </Button>
