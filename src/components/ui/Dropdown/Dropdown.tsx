      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            style={{ 
              ...(align === 'center' ? { left: '50%' } : { [align]: 0 }), 
              ...(direction === 'up' ? { bottom: 'calc(100% + 8px)', top: 'auto', transformOrigin: `bottom ${align}` } : {})
            }}
            initial={{ opacity: 0, scale: 0.95, y: direction === 'up' ? 10 : -10, x: align === 'center' ? '-50%' : 0 }}
            animate={{ opacity: 1, scale: 1, y: 0, x: align === 'center' ? '-50%' : 0 }}
            exit={{ opacity: 0, scale: 0.95, y: direction === 'up' ? 10 : -10, x: align === 'center' ? '-50%' : 0 }}
